...
static double q3_calcular_diametro_eixo_flexo_torcao(double M_res_lbin, double T_lbin, double Kt,
    ...
    if (fabs(M_res_lbin) < 1e-6 && fabs(T_lbin) < 1e-6) return 0.0;
    if (fabs(M_res_lbin) < 1e-6) tf_sq = 0;
    if (fabs(T_lbin) < 1e-6) tt_sq = 0;
...
void q3(FILE *fp_respostas, FILE *fp_dados) {
    ...
    if (fabs(Q3_X_E - Q3_X_B) < 1e-6) { R_Ey_lb = 0; R_Ez_lb = 0;} // Evita divisao por zero
    else {
    ...
    T_lbin_val = t_eixo_AB_lbin; Vy_lbf_val = F_A.Fy_lb; Vz_lbf_val = F_A.Fz_lb; Mz_lbin_val = 0.0; My_lbin_val = 0.0;
    ...
    // Ponto antes de B
    x_in = Q3_X_B - epsilon; T_lbin_val = t_eixo_AB_lbin;
    Mz_lbin_val = F_A.Fy_lb * (x_in - Q3_X_A); My_lbin_val = F_A.Fz_lb * (x_in - Q3_X_A);
    fprintf(fp_dados, "%.4f %.2f %.2f %.2f %.2f %.2f\n", x_in*POLEGADA_PARA_METRO, T_lbin_val*LBIN_PARA_NM, Mz_lbin_val*LBIN_PARA_NM, Vy_lbf_val*LBF_PARA_NEWTON, My_lbin_val*LBIN_PARA_NM, Vz_lbf_val*LBF_PARA_NEWTON);

    // Ponto B (Mancal)
    x_in = Q3_X_B; T_lbin_val = t_eixo_BC_lbin; // Torque continua t_eixo_AB = t_eixo_BC
    Mz_lbin_val = F_A.Fy_lb * (x_in - Q3_X_A); My_lbin_val = F_A.Fz_lb * (x_in - Q3_X_A);
    ...
    Vy_lbf_val += R_By_lb; Vz_lbf_val += R_Bz_lb;
    ...
    T_lbin_val = t_eixo_CD_lbin; Vy_lbf_val += F_C_eng.Fy_lb; Vz_lbf_val += F_C_eng.Fz_lb;
    ...
    // Ponto antes de D
    x_in = Q3_X_D - epsilon; T_lbin_val = t_eixo_CD_lbin;
    ...
    x_in = Q3_X_D; T_lbin_val = t_eixo_CD_lbin;
    Mz_lbin_val = F_A.Fy_lb*(x_in-Q3_X_A) + R_By_lb*(x_in-Q3_X_B) + F_C_eng.Fy_lb*(x_in-Q3_X_C);
    My_lbin_val = F_A.Fz_lb*(x_in-Q3_X_A) + R_Bz_lb*(x_in-Q3_X_B) + F_C_eng.Fz_lb*(x_in-Q3_X_C);
    fprintf(fp_dados, "%.4f %.2f %.2f %.2f %.2f %.2f\n", x_in*POLEGADA_PARA_METRO, T_lbin_val*LBIN_PARA_NM, Mz_lbin_val*LBIN_PARA_NM, Vy_lbf_val*LBF_PARA_NEWTON, My_lbin_val*LBIN_PARA_NM, Vz_lbf_val*LBF_PARA_NEWTON); // Antes
    T_lbin_val = t_eixo_DE_lbin; Vy_lbf_val += F_D_roda.Fy_lb; Vz_lbf_val += F_D_roda.Fz_lb;
    fprintf(fp_dados, "%.4f %.2f %.2f %.2f %.2f %.2f\n", x_in*POLEGADA_PARA_METRO, T_lbin_val*LBIN_PARA_NM, Mz_lbin_val*LBIN_PARA_NM, Vy_lbf_val*LBF_PARA_NEWTON, My_lbin_val*LBIN_PARA_NM, Vz_lbf_val*LBF_PARA_NEWTON); // Apos

    // Ponto antes de E
    x_in = Q3_X_E - epsilon; T_lbin_val = t_eixo_DE_lbin;
    ...
    // Ponto E (Mancal)
    x_in = Q3_X_E; T_lbin_val = t_eixo_DE_lbin; // Torque já deve ser zero ou próximo
    ...
    fprintf(fp_dados, "%.4f %.2f %.2f %.2f %.2f %.2f\n", x_in*POLEGADA_PARA_METRO, T_lbin_val*LBIN_PARA_NM, Mz_lbin_val*LBIN_PARA_NM, Vy_lbf_val*LBF_PARA_NEWTON, My_lbin_val*LBIN_PARA_NM, Vz_lbf_val*LBF_PARA_NEWTON); // Antes
    Vy_lbf_val += R_Ey_lb; Vz_lbf_val += R_Ez_lb; // Cortantes devem zerar
